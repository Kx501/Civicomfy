...
async def _get_civitai_model_and_version_details(api: CivitaiAPI, model_url_or_id: str, req_version_id: Optional[int]) -> Dict[
    str, Any]:
    """
    ...
    if not target_model_id and potential_version_id_from_input:
        ...
        if temp_version_info and "error" not in temp_version_info and temp_version_info.get('modelId'):
            ...
            err = temp_version_info.get('details', 'Could not find model ID from version') if isinstance(temp_version_info,
                                                                                                         dict) else 'API error'
            raise web.HTTPNotFound(reason=f"Could not determine Model ID from Version ID {potential_version_id_from_input}",
                                   body=json.dumps({"error": f"Version {potential_version_id_from_input} not found or missing modelId",
                                                    "details": err}))
    ...
    if not model_info_result or "error" in model_info_result:
        err_details = model_info_result.get('details', 'Unknown API error') if isinstance(model_info_result,
                                                                                          dict) else 'Unknown API error'
        raise web.HTTPNotFound(reason=f"Model {target_model_id} not found or API error",
    ...
    if potential_version_id_from_input:
        ...
        if 'temp_version_info' in locals() and temp_version_info.get('id') == target_version_id:
            ...
            if not version_info_result or "error" in version_info_result:
                err_details = version_info_result.get('details', 'Unknown API error') if isinstance(version_info_result,
                                                                                                    dict) else 'Unknown API error'
                raise web.HTTPNotFound(reason=f"Specified Version {target_version_id} not found or API error", body=json.dumps(
                    {"error": f"Version {target_version_id} not found or API error", "details": err_details}))
        ...
        if not version_info_result or "error" in version_info_result:
            ...
            err_details = version_info_result.get('details', 'Unknown error getting full version') if isinstance(version_info_result,
                                                                                                                 dict) else 'Error'
            print(
                f"[API Helper] Warning: Could not fetch full details for latest version {target_version_id}. Details: {err_details}. Falling back to summary.")
            ...
            version_info_to_use['model'] = version_info_to_use.get('model', {
                'name': model_info.get('name', 'Unknown')})  # Add fallback model name
    ...
    if not files:
        raise web.HTTPNotFound(
            reason=f"Version {target_version_id} (Name: {version_info_to_use.get('name', 'N/A')}) has no files listed.")
...
async def route_get_model_details(request):
    ...
    try:
        ...
        try:
            ...
            pass
        return web.json_response(
            {"success": False, "error": http_err.reason, "details": body_detail or "No details", "status_code": http_err.status},
        ...
        print("--- End Error ---")
        return web.json_response(
            {"success": False, "error": "Internal Server Error", "details": f"An unexpected error occurred: {str(e)}",
...
async def route_download_model(request):
    ...
    try:
            ...
            if version_info_result and "error" not in version_info_result:
                ...
                if not target_model_id and version_info.get('modelId'):
                    ...
                    if model_info_result and "error" not in model_info_result:
                        ...
                        print(
                            f"[Server Download] Warning: Could not fetch model info ({target_model_id}) after inferring from version.")
                ...
                raise web.HTTPNotFound(
                    reason=f"Civitai API Error: Version {target_version_id} not found or API error. Details: {err_details}",
            ...
            if model_info_result and "error" not in model_info_result:
                ...
                if versions and isinstance(versions, list) and len(versions) > 0:
                    ...
                    if full_version_info_result and "error" not in full_version_info_result:
                        ...
                        err_details = full_version_info_result.get('details') if isinstance(full_version_info_result,
                                                                                            dict) else "Unknown error getting full version"
                        print(
                            f"[Server Download] Warning: Could not fetch full details for version {target_version_id}. Details: {err_details}. Download might fail if file info is missing.")
                ...
                raise web.HTTPNotFound(
                    reason=f"Civitai API Error: Model {target_model_id} not found or API error. Details: {err_details}",
                    body=json.dumps({"error": f"Model {target_model_id} not found or API error", "details": err_details}))
        ...
        if not files and 'downloadUrl' in version_info and version_info['downloadUrl']:
            print(
                "[Server Download] Warning: No 'files' array found, attempting to use version-level 'downloadUrl'. File details will be incomplete.")
        ...
        if not files:
            raise web.HTTPNotFound(
                reason=f"Version ID {target_version_id} ({version_info.get('name', 'N/A')}) has no files listed in API response.")
        ...
        print(
            f"[Server Download] Selected file: Name='{primary_file.get('name', 'N/A')}', Type='{primary_file.get('type', 'N/A')}', SizeKB={primary_file.get('sizeKB')}, Format={primary_file.get('metadata', {}).get('format')}, Size={primary_file.get('metadata', {}).get('size')}")
        ...
        if custom_filename:
            ...
            if not ext:
                ...
                if api_ext and api_ext.lower() in valid_extensions:
                    ...
                    print(
                        f"[Server Download] Warning: Custom filename lacked extension, defaulting to '{ext}' based on API info or guess.")
            elif ext.lower() not in valid_extensions:
                print(
                    f"[Server Download] Warning: Custom filename has unusual extension '{ext}'. Proceeding, but ensure it's correct.")
        ...
        # Handle custom filename with relative path support
        if custom_filename and ('/' in custom_filename or '\\' in custom_filename):
            # Custom filename contains path separators - treat as relative path
            # Split the custom filename into path and filename parts
            custom_path_parts = custom_filename.replace('\\', '/').split('/')
            if len(custom_path_parts) > 1:
                # Extract the filename (last part) and path (all but last part)
                custom_filename_only = custom_path_parts[-1]
                custom_path_only = '/'.join(custom_path_parts[:-1])

                # Sanitize the filename part
                base, ext = os.path.splitext(custom_filename_only)
                sanitized_base = sanitize_filename(base, default_filename=f"model_{target_model_id}_custom")
                ...
                # Create the custom subdirectory within the model type directory
                custom_subdir = os.path.join(output_dir, custom_path_only)
                try:
                    os.makedirs(custom_subdir, exist_ok=True)
                    print(f"[Server Download] Created custom subdirectory: {custom_subdir}")
                except OSError as e:
                    print(f"[Server Download] Warning: Could not create custom subdirectory '{custom_subdir}': {e}")
                    # Fallback to base directory
                    custom_subdir = output_dir

                output_path = os.path.join(custom_subdir, final_filename)
            else:
                # Single filename without path
                output_path = os.path.join(output_dir, final_filename)
        else:
            # No path in custom filename, use base directory
            output_path = os.path.join(output_dir, final_filename)
        ...
        try:
            ...
            pass  # Ignore parsing errors, just use the raw text
        ...
        return web.json_response(
            {"error": "Internal Server Error", "details": f"An unexpected error occurred: {str(e)}", "status_code": 500}, status=500)
...
async def route_get_status(request):
    ...
    try:
        ...
        return web.json_response({"error": "Internal Server Error", "details": f"Failed to get status: {str(e)}", "status_code": 500},
                                 status=500)
...
async def route_cancel_download(request):
    ...
    try:
        ...
        return web.json_response(
            {"error": "Internal Server Error", "details": f"Failed to cancel download: {str(e)}", "status_code": 500}, status=500)
...
async def route_search_models(request):
    ...
    try:
        ...
        print("--- End Error ---")
        return web.json_response(
            {"error": "Internal Server Error", "details": f"An unexpected search error occurred: {str(e)}", "status_code": 500},
            status=500)
...
async def route_retry_download(request):
    ...
    try:
        ...
        if not download_id:
            return web.json_response(
                {"error": "Missing 'download_id'", "details": "The request body must contain the 'download_id' of the item to retry."},
                status=400)
...
async def route_open_path(request):
    ...
    try:
        ...
        if not download_id:
            return web.json_response({"error": "Missing 'download_id'",
                                      "details": "The request body must contain the 'download_id' of the completed item."}, status=400)
        ...
        status_code = 200 if result.get("success") else 404 if "not found" in result.get("error",
                                                                                         "").lower() else 400  # Use 400 for OS error, security etc
