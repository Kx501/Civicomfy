...
class CivitaiDownloaderUI {
            ...
            <div class="civitai-downloader-modal-content">
                ...
                <div class="civitai-downloader-body">
                    <div class="civitai-downloader-tabs">
                        <button class="civitai-downloader-tab active" data-tab="download">下载</button>
                        <button class="civitai-downloader-tab" data-tab="search">搜索</button>
                        <button class="civitai-downloader-tab" data-tab="status">状态 <span id="civitai-status-indicator" style="display: none;">(<span id="civitai-active-count">0</span>)</span></button>
                        <button class="civitai-downloader-tab" data-tab="settings">设置</button>
                    </div>
                    <div id="civitai-tab-download" class="civitai-downloader-tab-content active">
                        <form id="civitai-download-form">
                            <div class="civitai-form-group">
                                <label for="civitai-model-url">模型URL或ID</label>
                                <input type="text" id="civitai-model-url" class="civitai-input" placeholder="例如：https://civitai.com/models/12345 或 12345" required>
                            </div>
                            <p style="font-size: 0.9em; color: #ccc; margin-top: -10px; margin-bottom: 15px;">您可以选择性地在URL中使用"?modelVersionId=xxxxx"或在下面的字段中指定版本ID。</p>
                            <div class="civitai-form-row">
                                <div class="civitai-form-group">
                                    <label for="civitai-model-type">模型类型（保存位置）</label>
                                    <select id="civitai-model-type" class="civitai-select" required></select>
                                </div>
                                <div class="civitai-form-group">
                                    <label for="civitai-model-version-id">版本ID（可选）</label>
                                    <input type="number" id="civitai-model-version-id" class="civitai-input" placeholder="覆盖URL/最新版本">
                                </div>
                            </div>
                             <div class="civitai-form-row">
                                <div class="civitai-form-group">
                                    <label for="civitai-custom-filename">自定义文件名（可选）</label>
                                    <input type="text" id="civitai-custom-filename" class="civitai-input" placeholder="留空使用原始名称，支持相对路径如：subfolder/filename">
                                </div>
                                <div class="civitai-form-group">
                                    <label for="civitai-connections">连接数</label>
                                    ...
                                    <p style="font-size: 0.9em; color: #ccc; margin-top: 7px; margin-bottom: 15px;">已禁用：目前仅支持单连接</p>
                                    </div>
                            </div>
                            <div class="civitai-form-group inline">
                                 <input type="checkbox" id="civitai-force-redownload" class="civitai-checkbox">
                                <label for="civitai-force-redownload">强制重新下载（如果已存在）</label>
                            </div>
                            ...
                            <button type="submit" id="civitai-download-submit" class="civitai-button primary">开始下载</button>
                        </form>
                    </div>
                        ...
                        <div id="civitai-status-content">
                             ...
                             <div class="civitai-status-section">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                     <h3>下载历史（最近）</h3>
                                     <button id="civitai-clear-history-button" class="civitai-button danger small" title="清除所有历史记录（包括本地文件）">
                        ...
                        <form id="civitai-settings-form">
                         <div class="civitai-settings-container">
                             <div class="civitai-settings-section">
                                 <h4>API和默认设置</h4>
                                <div class="civitai-form-group">
                                    <label for="civitai-settings-api-key">Civitai API密钥（可选）</label>
                                    <input type="password" id="civitai-settings-api-key" class="civitai-input" placeholder="输入API密钥以获得更高限制/认证访问" autocomplete="new-password">
                                    <p style="font-size: 0.85em; color: #bbb; margin-top: 5px;">某些下载/功能需要。在civitai.com/user/account查找密钥</p>
                                </div>
                                <div class="civitai-form-group">
                                    <label for="civitai-settings-connections">默认连接数</label>
                ...
                <div id="civitai-toast" class="civitai-toast"></div>
                 ...
                 <div id="civitai-confirm-clear-modal" class="civitai-confirmation-modal">
                     <div class="civitai-confirmation-modal-content">
                         <h4>确认清除历史</h4>
                         <p>您确定要清除下载历史吗？此操作无法撤销。</p>
                         <div class="civitai-confirmation-modal-actions">
                              <button id="civitai-confirm-clear-no" class="civitai-button secondary">取消</button>
                              <button id="civitai-confirm-clear-yes" class="civitai-button danger">确认清除</button>
                         </div>
                     </div>
                 </div>
        ...
        if (this.confirmClearModal && this.confirmClearYesButton && this.confirmClearNoButton) {
            ...
            this.confirmClearYesButton.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (result.success) {
                        this.showToast(result.message || '历史清除成功！', 'success');
                        this.statusData.history = []; // Clear in-memory state
                        this.renderDownloadList(this.statusData.history, this.historyListContainer, '暂无下载历史。');
                        this.confirmClearModal.style.display = 'none'; // Hide modal
                    } else {
                        this.showToast(`清除历史失败: ${result.details || result.error || '未知错误'}`, 'error', 5000);
                    }
                } catch (error) {
                    const message = `清除历史失败: ${error.details || error.message || '网络错误'}`;
                    ...
                    this.confirmClearYesButton.textContent = '确认清除';
                }
            });
        ...
        this.statusContent.addEventListener('click', async (event) => {
            ...
            // Handle Retry
                ...
                try {
                    ...
                    if (result.success) {
                        this.showToast(result.message || `重试已成功排队！`, 'success');
                        ...
                        this.showToast(`重试失败: ${result.details || result.error || '未知错误'}`, 'error', 5000);
                        button.disabled = false;
                        button.innerHTML = '<i class="fas fa-redo"></i>';
                        button.title = "重试下载";
                    }
                } catch (error) {
                    const message = `重试失败: ${error.details || error.message || '网络错误'}`;
            ...
            // Handle Open Path
                ...
                try {
                    ...
                    if (result.success) {
                        this.showToast(result.message || `路径打开成功！`, 'success');
                    } else {
                        this.showToast(`打开路径失败: ${result.details || result.error || '未知错误'}`, 'error', 5000);
                    }
                } catch (error) {
                    const message = `打开路径失败: ${error.details || error.message || '网络错误'}`;
        ...
        this.searchResultsContainer.addEventListener('click', (event) => {
            ...
            if (downloadButton) {
                ...
                if (!modelId || !versionId) {
                    ...
                    this.showToast("错误：缺少下载数据。", "error");
                    return;
                }
                ...
                if (versionsContainer) {
                    ...
                    if (currentlyVisible) {
                        ...
                        viewAllButton.innerHTML = `所有版本 (${viewAllButton.dataset.totalVersions}) <i class="fas fa-chevron-down"></i>`;
                        viewAllButton.title = `显示所有 ${viewAllButton.dataset.totalVersions} 个版本`;
                    } else {
                        versionsContainer.style.display = 'flex'; // Or 'block'
                        viewAllButton.innerHTML = `显示更少 <i class="fas fa-chevron-up"></i>`;
                        viewAllButton.title = `显示更少版本`;
                    }
                } else {
    ...
    async fetchAndDisplayDownloadPreview() {
        ...
        this.downloadPreviewArea.innerHTML = '<p><i class="fas fa-spinner fa-spin"></i> 正在加载模型详情...</p>';
        ...
        try {
            ...
            if (result && result.success) {
                ...
                const message = `获取详情失败: ${result.details || result.error || '未知后端错误'}`;
            ...
            const message = `获取详情时出错: ${error.details || error.message || '未知错误'}`;
        ...
        const previewHtml = `
            <div class="civitai-search-item" style="background-color: var(--comfy-input-bg);"> <!-- Style override for clarity -->
                ...
                <div class="civitai-search-info">
                    ...
                    <p style="font-weight: bold;">版本: ${versionName} <span class="base-model-badge" style="margin-left: 5px;">${baseModel}</span></p>

                    <div class="civitai-search-stats" title="Stats: Downloads / Rating (Count) / Likes">
                        <span title="下载"><i class="fas fa-download"></i> ${stats.downloads?.toLocaleString() || 0}</span>
                        <span title="喜欢"><i class="fas fa-thumbs-up"></i> ${stats.likes?.toLocaleString(0) || 0}</span>
                        <span title="不喜欢"><i class="fas fa-thumbs-down"></i> ${stats.dislikes?.toLocaleString() || 0}</span>
                        <span title="热度"><i class="fas fa-bolt"></i> ${stats.buzz?.toLocaleString() || 0}</span>
                    </div>
                    ...
                    <p style="font-size: 0.9em; color: #ccc;">
                     ...
                     <a href="${civitaiLink}" target="_blank" rel="noopener noreferrer" class="civitai-button small" title="在Civitai网站上查看" style="margin-top: 5px; display: inline-block;">
                        在Civitai查看 <i class="fas fa-external-link-alt"></i>
                    </a>
                ...
                <div style="margin-top: 15px;">
                     <h5 style="margin-bottom: 5px;">模型描述:</h5>
                ...
                <div style="margin-top: 15px;">
                     <h5 style="margin-bottom: 5px;">版本描述:</h5>
                     <div class="model-description-content" style="max-height: 200px; overflow-y: auto; background-color: var(--comfy-input-bg); padding: 10px; border-radius: 4px; font-size: 0.9em; border: 1px solid var(--border-color, #555);">
    ...
    async handleDownloadSubmit() {
        if (this.settings.apiKey == null || this.settings.apiKey == "") {
            this.showToast("API密钥为空，请在设置中填写您的API密钥", "error");
            this.switchTab("settings")
        }
        else {
            this.downloadSubmitButton.disabled = true;
            this.downloadSubmitButton.textContent = '正在启动...';

            const modelUrlOrId = this.modelUrlInput.value.trim();
            if (!modelUrlOrId) {
                this.showToast("模型URL或ID不能为空。", "error");
                this.downloadSubmitButton.disabled = false;
                this.downloadSubmitButton.textContent = '开始下载';
                return;
            }
            ...
            if (isNaN(params.num_connections) || params.num_connections < 1 || params.num_connections > 16) {
                this.showToast("连接数无效（必须是1-16）。", "error");
                this.downloadSubmitButton.disabled = false;
                this.downloadSubmitButton.textContent = '开始下载';
                return;
            }
            ...
            try {
                ...
                if (result.status === 'queued') {
                    this.showToast(`下载已排队: ${result.details?.filename || 'Model'}`, 'success');
    ...
    async handleSearchSubmit() {
        ...
        this.searchSubmitButton.textContent = '搜索中...';
        this.searchResultsContainer.innerHTML = '<p><i class="fas fa-spinner fa-spin"></i> 搜索中...</p>';
        ...
        try {
            ...
            this.searchSubmitButton.textContent = '搜索';
        }
    }
    ...
    async handleCancelDownload(downloadId) {
        ...
        try {
            ...
            this.showToast(result.message || `已请求取消 ${downloadId}`, 'info');
            // Status update should happen automatically via polling, but trigger one now for faster UI feedback
            this.updateStatus();

        } catch (error) {
            const message = `取消失败: ${error.details || error.message || '未知错误'}`;
            ...
            if (failedButton) {
                ...
                failedButton.innerHTML = '取消'; // Restore text
                failedButton.title = "取消下载";
            }
        }
    }
        ...
        try {
            ...
            if (newStatusData && Array.isArray(newStatusData.active) && Array.isArray(newStatusData.queue) && Array.isArray(newStatusData.history)) {
                ...
                if (this.activeTab === 'status') {
                    this.renderDownloadList(this.statusData.active, this.activeListContainer, '无活动下载。');
                    this.renderDownloadList(this.statusData.queue, this.queuedListContainer, '下载队列为空。');
                    // Render history using the data received directly from the API
                    this.renderDownloadList(this.statusData.history, this.historyListContainer, '尚无下载记录。');
                }

            } else {
                console.warn("[Civicomfy] Received invalid status data structure:", newStatusData);
                if (this.activeTab === 'status') { this.showStatusError("从服务器接收到无效数据。"); }
            }
        } catch (error) {
            console.error("[Civicomfy] Failed to update status:", error);
            if (this.activeTab === 'status') { this.showStatusError(`加载状态失败: ${error.details || error.message}`); }
        }
    }
    ...
    renderDownloadList(items, container, emptyMessage) {
        ...
        items.forEach(item => {
            ...
            switch (status) {
                case 'completed': progressBarClass = 'completed'; statusText = '已完成'; break;
                case 'failed': progressBarClass = 'failed'; statusText = '失败'; break; // Shorten status text
                case 'cancelled': progressBarClass = 'cancelled'; statusText = '已取消'; break; // Shorten status text
                case 'downloading': statusText = '下载中'; break;
                case 'queued': statusText = '排队中'; break;
                case 'starting': statusText = '启动中'; break;
            }
            ...
            const addedTooltip = addedTime ? `data-tooltip="添加时间: ${new Date(addedTime).toLocaleString()}"` : '';
            const startedTooltip = startTime ? `data-tooltip="开始时间: ${new Date(startTime).toLocaleString()}"` : '';
            const endedTooltip = endTime ? `data-tooltip="结束时间: ${new Date(endTime).toLocaleString()}"` : '';
            const durationTooltip = startTime && endTime ? `data-tooltip="耗时: ${this.formatDuration(startTime, endTime)}"` : '';
            const filenameTooltip = filename !== 'N/A' ? `title="文件名: ${filename}"` : '';
            // Limit error tooltip length for performance/display
            const errorTooltip = errorMsg ? `title="错误详情: ${String(errorMsg).substring(0, 200)}${String(errorMsg).length > 200 ? '...' : ''}"` : '';
            const connectionInfoHtml = connectionType !== "N/A" ? `<span style="font-size: 0.85em; color: #aaa; margin-left: 10px;">(连接: ${connectionType})</span>` : '';
            ...
            let innerHTML = `
                ...
                <div class="civitai-download-info">
                    ...
                    ${size > 0 ? `<p>大小: ${this.formatBytes(size)}</p>` : ''}
            ...
            if (status === 'downloading' || status === 'starting' || status === 'completed') {
                const statusLine = `<div ${durationTooltip} ${endedTooltip}>状态: ${statusText} ${connectionInfoHtml}</div>`;
                innerHTML += `
                ...
                innerHTML += `<div class="status-line-simple" ${durationTooltip} ${endedTooltip} ${addedTooltip}>状态: ${statusText} ${connectionInfoHtml}</div>`;
            } else {
                innerHTML += `<div class="status-line-simple">状态: ${statusText} ${connectionInfoHtml}</div>`;
            }
            ...
            if (status === 'queued' || status === 'downloading' || status === 'starting') {
                innerHTML += `<button class="civitai-button danger small civitai-cancel-button" data-id="${id}" title="取消下载"><i class="fas fa-times"></i></button>`;
            }
            // --- Enable Retry Button ---
            if (status === 'failed' || status === 'cancelled') {
                // REMOVED disabled attribute and updated title
                innerHTML += `<button class="civitai-button small civitai-retry-button" data-id="${id}" title="重试下载"><i class="fas fa-redo"></i></button>`;
            }
            // --- Add Open Path Button ---
            if (status === 'completed') {
                // ADDED this button
                innerHTML += `<button class="civitai-button small civitai-openpath-button" data-id="${id}" title="打开包含文件夹"><i class="fas fa-folder-open"></i></button>`;
            }
    ...
    async populateBaseModels() {
        ...
        try {
            ...
            this.showToast('加载基础模型列表失败', 'error');
        }
    }
        ...
        items.forEach(hit => {
            ...
            const moreButtonHtml = hasMoreVersions ? `
                <button class="civitai-button secondary small show-all-versions-button"
                        ...
                        title="显示所有 ${totalVersionCount} 个版本">
                    所有版本 (${totalVersionCount}) <i class="fas fa-chevron-down"></i>
                </button>
            ` : '';
            ...
            listItem.innerHTML = `
                ...
                <div class="civitai-search-actions">
                    <a href="https://civitai.com/models/${modelId}${primaryVersionId ? '?modelVersionId=' + primaryVersionId : ''}" target="_blank" rel="noopener noreferrer" class="civitai-button small" title="在Civitai网站上查看">
